<template>
	<view class="container">
		...
		<view class="btnGroup">
			...
			<view class="btn toList" @click="toList(code)" v-if="checkNumberInArray(menu,'8')">
				查看物资调用申请
			</view>
		</view>
	</view>
	...
	export default {
		...
		methods: {
			...
			toAdd(code){
				...
				setTimeout(function() {
					uni.navigateTo({
							url:'../addOrder/addOrder?code=' + code + '&address=' + address
						}
				)}, 300);
			},
			toList(code){
				...
				setTimeout(function() {
					uni.navigateTo({
							url:'../orderList/orderList?code=' + code + '&address=' + address
						}
				)}, 300);
			}
		},
