...
import com.app.device.domain.SysLog.SysLogPo;
...
public class ServerThread extends Thread
{
    ...
    public void start()
    {
        if (m_isRuning)
        {
            log.error(">>>线程" + this.getId() + "启动失败,该线程正在执行");
            return;
        }
        else
        {
        ...
        try
        {
            ...
            while (m_isRuning)
            {
                ...
                if (inputStream.available() > 0)
                {
                    ...
                    if (inputStream.available() == 0)
                    {
                        log.info(">>>线程" + this.getId() + "收到:" + info);
                        ...
                        //重置,不然每次收到的数据都会累加起来
                        info = "";
                        log.info(">>>线程" + this.getId() + "回应:" + responseStr);
                    }
                }
            }
        }
    ...
    private void addLog(String info){
        try {
            ...
            sysLogPo.setType(BaseConst.LOG_TYPE_HT);
            sysLogPo.setMsg(info);
            sysLogPo.setParam("");
            sysLogPo.setReturnValue("");
            sysLogPo.setModule("token校验");
            sysLogPo.setMethod("ServerThread");
            sysLogPo.setIp("127.0.0.1");
